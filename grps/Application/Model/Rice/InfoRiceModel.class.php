...
abstract class InfoRiceModel extends Model{
	...
	protected $_validateForBRice = array(
		...
		array('year','4','年份应为4位正整数',self::EXISTS_VALIDATE,'length'),
	...
	*/
	public function addRice($data,$riceKind){
		...
		if ($this->validate($validateRule)->create($data)){
			...
			if($id = $this->where("dist_id = '$data[dist_id]' and year = '$data[year]'")->getField('rice_id')){//一条记录由由年份和地区标记
				$this->where("rice_id='$id'")->save();
			}
			else if($id=$this->add($data)){
				;//空
			}
			else{
				$this->error = "添加水稻失败,未知原因1!";
				return false; //添加失败
			}
	...
	protected function _calculate($data, $data1){
		...
		$data['al_planting_stru'] =  $data['l_sown_area']*1.0 / $data['total_sown_area'] ;  //晚稻播种结构(0.01%) = 晚稻播种面积(0.01亩)/ 晚稻总播种面积(0.01亩)
		$data['al_area_disasterr'] =  $data['l_disaster_area'] * 1.0/ $data['l_sown_area'];  //晚稻面积受灾率(0.01%) = 晚稻受灾面积(0.01亩)/  晚稻播种面积(0.01亩)
	...
	*/
	public function setRiceState($rice_id,$state,$riceKind){
		...
		$riceStatus = $this->getRiceStatus($state);//获取水稻信息状态从系统逻辑设置到数据库存储值的映射
		if($riceStatus==-1){
