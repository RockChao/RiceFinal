...
abstract class InfoUserModel extends Model{
 	...
 	protected $_validate = array(
 		...
 		array('user_name','4,20',"用户名长度必须为1到20个字符",self::EXISTS_VALIDATE,'length'),//用户名长度不合法,
	...
	*/
	public function login($data){
		...
		if (is_array($result)){
			if ($result["user_pw"] == $user_pw){
				...
				session("dist_id", $result["user_region"]);
	...
	public function logout(){
		...
		session("dist_id", null);
		session("dist_latitude", null);
    	session("dist_longitude", null);
	}
